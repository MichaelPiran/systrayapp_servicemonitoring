...
// 	myWindow := myApp.NewWindow(fmt.Sprintf("%s event viewer", serviceMonitoredName))
...
// 		cmd := exec.Command("powershell", "-Command", "Get-EventLog", "-LogName", "Application", "-Source", serviceMonitoredName, "-Newest", lastEventCount, "| Select-Object @{Name='TimeGenerated';Expression={$_.TimeGenerated.ToString('yyyy-MM-dd HH:mm:ss')}}, Message | ConvertTo-Json")
